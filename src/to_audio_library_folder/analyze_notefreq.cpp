/*
    modified pitch estimation object to work with block size of 1024 (instead of default 128)
*/
#include <Arduino.h>
...
#define HALF_BLOCKS AUDIO_GUITARTUNER_BLOCKS * AUDIO_BLOCK_SAMPLES / 2
...
void AudioAnalyzeNoteFrequency::update( void ) {
    ...
    if ( state >= AUDIO_GUITARTUNER_BLOCKS ) {
        if ( next_buffer ) {
            if ( !first_run && process_buffer ) process( );
            for ( int i = 0; i < AUDIO_GUITARTUNER_BLOCKS; i++ ) copy_buffer( AudioBuffer+( i * AUDIO_BLOCK_SAMPLES ), blocklist1[i]->data );
            for ( int i = 0; i < AUDIO_GUITARTUNER_BLOCKS; i++ ) release( blocklist1[i] );
            next_buffer = false;
        } else {
            if ( !first_run && process_buffer ) process( );
            for ( int i = 0; i < AUDIO_GUITARTUNER_BLOCKS; i++ ) copy_buffer( AudioBuffer+( i * AUDIO_BLOCK_SAMPLES ), blocklist2[i]->data );
...
void AudioAnalyzeNoteFrequency::process( void ) {
    ...
    //for(int rep = 0; rep < 8; rep++){
        uint16_t cycles = AUDIO_BLOCK_SAMPLES/2;
